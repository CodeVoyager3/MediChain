...
export const DialogStack = ({ stack, trigger }) => {
  ...
  return (
    <div
      className="min-h-[450px] sm:min-h-[600px] flex flex-col items-center justify-center">
      <motion.button
      ...
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
              className="relative w-xs sm:w-sm flex items-center justify-center min-h-[450px] sm:min-h-[500px]">
                ...
                {stack.map((item, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      {/* Header */}
                      <div
                        className="flex items-center justify-between bg-neutral-50 dark:bg-neutral-800 border-b-[1.5px] border-neutral-200 dark:border-neutral-700 px-4 sm:px-5 py-2.5 sm:py-3 transition-colors">
                        <h3
                          className="text-base sm:text-lg font-medium text-neutral-500 dark:text-neutral-400">
                          {item.title}
                        </h3>
                        <button
                          title="close"
                          ...
                          <X
                            size={20}
                            className="sm:size-[22px] text-neutral-500 dark:text-neutral-400" />
                        </button>
                      </div>
                      {/* Content */}
                        ...
                        {item.type === 'form' ? (
                          <div className="space-y-4 sm:space-y-5">
                            <div className="space-y-2.5 sm:space-y-3">
                              <label
                                className="block text-sm sm:text-base font-normal text-neutral-600 dark:text-neutral-400">
                                Email Address
                              </label>
                              <input
                                title="email"
                            ...
                            <button
                              className="w-full bg-black dark:bg-white text-white dark:text-black py-3.5 sm:py-4 rounded-xl sm:rounded-2xl font-semibold flex items-center justify-center gap-2 transition-colors active:scale-[0.98]">
                            ...
                            <div className="space-y-5 sm:space-y-6">
                              {item.steps?.map((step, i) => (
                                <div key={i} className="flex gap-3 sm:gap-4 items-start group">
                                  <div
                                    className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl border flex items-center justify-center shrink-0 transition-colors bg-neutral-100 dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 text-neutral-800 dark:text-neutral-200">
                                    <HugeiconsIcon
                                      icon={step.icon}
                                  ...
                                  <p
                                    className="text-sm sm:text-base leading-snug pt-0.5 sm:pt-1 text-neutral-700 dark:text-neutral-300">
                                    {step.text}
                                  </p>
                                </div>
                              ))}
