...
export default function InsuranceDashboard() {
    ...
    return (
        <DashboardLayout
            title="Claims & Fraud Engine"
            user={{ name: user?.name || 'Insurer', email: user?.walletAddress?.slice(0, 10) + '...', avatar: "" }}
            navMain={navMain}
            navSecondary={navSecondary}
        >
            {verifyError && (
                <div className="bg-red-950/30 text-red-300 text-xs p-3 rounded-xl flex justify-between border border-red-900/50 mb-4">
